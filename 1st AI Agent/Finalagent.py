...
budget_guardrail_agent = Agent(
    ...
    instructions=(
        ...
        "If the request says things  too low "
...
async def main():
    try:
        ...
        session = SQLiteSession("travel_agent",db_path="travel_agent.sqlite")

        # First turn
        result = await Runner.run(
            travel_agent,
            '''I think will go to Dubai in feb, with $1500 budget , suggest plan''',
            session=session
        )
